...
def load_contacts():
    try:
        ...
        # close the student_data.dat file
        input_file.close()
    except IOError:
...
def get_menu_choice():
    ...
    print("(5) Print all student information (this will save changes).")
...
def change_gpa(mycontacts):
    ...
    if name in mycontacts:
        ...
        grade = input("Enter expected grade here: ")
        status = input("Status (full-time or part-time): ")
...
def print_all(mycontacts):
    # This will save all working changes to student files
    save_contacts(mycontacts)

    for val in mycontacts.values():
        print()
        print(val)
        print()
